...
- Financial services have the `staging-automation` profile and remain stopped until the disposable test wallets, two independent RPCs, transfer history and launch plan are ready. The active owner-wallet route does not require a governance executor or vault program
...
Run `npm run preflight:solana-web` on the staging host, or on a local machine while the SSH tunnel to `127.0.0.1:8788` is open. This read-only diagnostic checks the web container's finalized RPC route, the offchain ballot API and that the historical transaction-vote route remains disabled. It never reads or prints the protected upstream URL, signs or broadcasts. `readOnlyRpcReady`, `offchainVotingApiReady` and `legacyTransactionVoteDisabled` must all be true. `voteEndToEndVerified` remains false until a real holder signs and the vote is counted on the test token. The backend `preflight:solana-test` uses `.env.solana` and does **not** verify the web container's `.env.web`. Run both diagnostics; neither substitutes for mobile-wallet or voting tests. After public deployment, use `npm run preflight:solana-web -- --public` to probe the canonical HTTPS site through its reverse proxy
...
Before the test holder vote, check an installed wallet's in-app browser, an external mobile browser with WalletConnect, and desktop QR connection. Verify connect, address, disconnect, account switching, wallet-app return, signature rejection, one accepted signed message, duplicate rejection, public tally and historical proposal link. No Solana vote transaction or deployed governance program is required. Never ask a holder to connect or sign merely to receive an automatic MSTRx reward. The historical onchain vote route remains disabled
...
The test marketing address may reuse the test owner. A marketing-sale choice in the offchain vote is advisory; the reserve-wallet owner must carry out any resulting sale and publish its transaction separately
