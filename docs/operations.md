...
The active holder ballot is offchain. The owner opens it with one signed control action in the private panel; the runner checks the reserve balance across two finalized RPCs and publishes a holder-history snapshot. Eligible wallets sign a message, not a transaction; the web service stores one receipt per wallet and publishes tallies. The reserve remains in the owner's separate wallet and is not code-locked by a ballot. A vote result is advisory and does not execute a reserve trade. Historical program-based vote and execution code remains unreleased
